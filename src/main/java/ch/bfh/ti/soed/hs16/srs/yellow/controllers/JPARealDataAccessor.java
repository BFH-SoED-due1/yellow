...
public class JPARealDataAccessor implements DataAccessor {
	...
	public Long authentifyCustomer(String login, String password) {
		...
		entr.begin();
		TypedQuery<Customer> query = this.entityManager.createQuery(
				"SELECT c FROM CustomerEntity c " + "WHERE c.cred.login = :login "
						+ "                                                           AND c.cred.passwordHash = :password",
				Customer.class);
